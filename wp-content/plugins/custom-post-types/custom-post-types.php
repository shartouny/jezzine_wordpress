...
if (!class_exists('adg_custom_post_types')) {
    class adg_custom_post_types
    {
        ...
        public function update_form_view_field_types($id, $type, $value, $required, $options)
        {
            ...
            if ($type == 'checkbox') { ?>
                <div class="checkbox-group">
                    <input type="hidden" name="custom_field[<?php echo $id; ?>][]" value="">
                    <?php
        ...
        public function update_form_edit_field_types($types)
        {
            ...
            // v 2.0.4 - add "required" on v 2.1.8
            function the_checkbox_field_output($id, $options = null)
            { ?>
                <label><?php _e('Field options', 'custom-post-types'); ?>
                    <textarea class="field-extra-options" name="field_settings[fields][<?php echo $id; ?>][options]" placeholder="<?php _e('Field options, one per row', 'custom-post-types'); ?>" data-required="yes"><?php echo $options; ?></textarea>
                </label>
            <?php }
            ...
            // v 2.1.7
            $types['code'] = array(
                'label' => __('TEXTAREA (no editor)', 'custom-post-types'),
            );
        ...
        public function update_shortcode_field_view($value, $field_type, $id)
        {
            ...
            // v 2.1.6
            if ($field_type == 'code') {
                $value = '<span class="field-' . $id . '">' . $value . '</span>';
            }
        ...
        public function update_theme_compatibility_template($themes)
        {
            ...
            $themes['hueman'] = array(
                'single_template' => get_template_directory() . '/single.php',
                'replace_type' => 'string',
                'replace' => "hu_get_content( 'tmpl/single-tmpl')",
            );
            $themes['aardvark'] = array(
                'single_template' => get_template_directory() . '/single.php',
                'replace_type' => 'string',
                'replace' => "get_template_part( 'lib/sections/single/post-content' )",
            );

            // v 2.1.4
            $themes['kadence'] = array(
                'single_template' => get_template_directory() . '/single.php',
                'replace_type' => 'string',
                'replace' => "do_action( 'kadence_single' )",
            );
            $themes['ascend'] = array(
                'single_template' => get_template_directory() . '/single.php',
                'replace_type' => 'string',
                'replace' => "get_template_part('templates/content', 'single')",
            );
            $themes['virtue'] = array(
                'single_template' => get_template_directory() . '/single.php',
                'replace_type' => 'string',
        ...
        {
            if (!$this->is_pro_version_active()) {
                return '<a class="button button-primary button-hero button-icon pro-icon" href="https://www.andreadegiovine.it/risorse/plugin/custom-post-types?utm_source=tools_plugin_page&amp;utm_medium=plugin_page&amp;utm_campaign=custom_post_types" target="_blank">' . __('Become PRO', 'custom-post-types') . '</a>';
            }
        }
        ...
        public function go_pro_content()
        {
            ...
            <p>The subscription has a duration of <strong>12 months</strong>.</p>', 'custom-post-types');
        ...
        public function init_admin_menu()
        {
            ...
            add_submenu_page('edit.php?post_type=' . $this->cpt_ui_name, __('Tools & Settings &lsaquo; Extend / Manage', 'custom-post-types'), __('Tools & Settings', 'custom-post-types'), 'administrator', 'tools', array($this, 'init_tools_page'));
        ...
        public function init_cpt_ui_metabox($post_type)
        {
            ...
            // Search for custom fields box for current post type
            ...
            foreach ($all_custom_metaboxes as $metaboxes) {
                ...
                $field_role = $field_settings['field_role'] == 0 ? 'edit_posts' : 'update_core';

                if (isset($field_settings['field_post_types']) && in_array($post_type, $field_settings['field_post_types']) && current_user_can($field_role)) {
        ...
        public function init_cpt_ui_save_metabox($post_id)
        {
            ...
            if (isset($_POST['custom_metabox_nonce'])) {
                ...
                $mydata = isset($_POST['custom_field']) ? $_POST['custom_field'] : array();
                if (is_array($mydata) && !empty($mydata)) {
                    foreach ($mydata as $meta => $value) {
                        if (is_array($value)) {
                            $value = array_values(array_filter($value)); // add on v 2.1.10
                        }
        ...
        public function init_created_cpt()
        {
            ...
            foreach ($created_post_types as $post_type) {
                ...
                $args = array(
                    ...
                    'rewrite'            => $cpt_settings['cpt_public'] == 0 ? false : array('slug' => sanitize_title($post_type->post_title), 'with_front' => false),
                    'capabilities' => array(
        ...
        public function init_created_tax()
        {
            ...
            foreach ($created_taxonomies as $taxonomy) {
                ...
                $args = array(
                    ...
                    'rewrite'           => $tax_settings['tax_public'] == 0 ? false : array('slug' => sanitize_title($taxonomy->post_title), 'with_front' => false),
        ...
        public function init_admin_notices()
        {
            ...
            if(isset($_GET['cpt_action']) && $_GET['cpt_action'] == 'hide_custom_post_types_support_project_notice' ){
                update_option('custom_post_types_support_project_notice', strtotime("+5 day", time()));
                $redirect_url = remove_query_arg('cpt_action', $current_full_url);
                printf('<script>window.location.replace("%s");</script>', $redirect_url);
            }

            $support_project_notice = get_option('custom_post_types_support_project_notice', false);

            if (!$support_project_notice || time() > $support_project_notice) {
                if (!$this->is_pro_version_active()) {
                    printf(
                        '<div class="%1$s"><p>%2$s<br><strong><a href="%3$s" target="_blank">%4$s</a></strong> - <a href="%5$s" target="_blank">%6$s</a> - <a href="%7$s" target="_blank">%8$s</a> - <a href="%9$s">%10$s</a></p></div>',
                        $this->cpt_ui_name . '-notices notice notice-success is-dismissible',
                        __('<strong>Custom post type</strong> notice:<br>Thanks for using this plugin! Do you want to help us grow to add new features?', 'custom-post-types'),
                        'https://www.andreadegiovine.it/risorse/plugin/custom-post-types?utm_source=tools_plugin_page&utm_medium=plugin_page&utm_campaign=custom_post_types',
                        __('Become PRO', 'custom-post-types'),
                        'https://wordpress.org/support/plugin/custom-post-types/reviews/#new-post',
                        __('Review', 'custom-post-types'),
                        'https://www.andreadegiovine.it/outlinks/1422/?utm_source=wordpress_dashboard&utm_medium=notices&utm_campaign=custom_post_types',
                        __('Send donation', 'custom-post-types'),
                        add_query_arg(['cpt_action' => 'hide_custom_post_types_support_project_notice'], $current_full_url),
                        __('Close and don\'t show for 5 days', 'custom-post-types')
                    );
                } else {
                    printf(
                        '<div class="%1$s"><p>%2$s<br><strong><a href="%3$s" target="_blank">%4$s</a></strong> - <a href="%5$s" target="_blank">%6$s</a> - <a href="%7$s">%8$s</a></p></div>',
                        $this->cpt_ui_name . '-notices notice notice-success is-dismissible',
                        __('<strong>Custom post type</strong> notice:<br>Thanks for using this plugin! Do you want to help us grow to add new features?', 'custom-post-types'),
                        'https://wordpress.org/support/plugin/custom-post-types/reviews/#new-post',
                        __('Review', 'custom-post-types'),
            ...
            if (isset($current_screen->post_type) && $current_screen->post_type == $this->cpt_ui_name . '_template') {
                $current_theme_name = wp_get_theme()->get('TextDomain');
                if (is_child_theme()) {
                ...
                if (!isset($theme_compatibility_array[$current_theme_name]) && empty($theme_single_template)) {
                    printf(
                        '<div class="%1$s"><p>%2$s<br><a href="%3$s" class="button-primary" style="margin-top:1em;" target="_blank">%4$s</a> <a href="%5$s" class="button-secondary" style="margin-top:1em;" target="_blank">%6$s</a></p></div>',
                        $this->cpt_ui_name . '-notices notice notice-error',
                        __('<strong>Custom post type</strong> notice:<br>The theme you are using does not contain the "single.php" file in the main theme folder. To use custom templates the theme must have the "single.php" file.', 'custom-post-types'),
                        'https://wordpress.org/support/plugin/custom-post-types/',
                        __('Ask for integration', 'custom-post-types'),
                        'https://developer.wordpress.org/themes/template-files-section/post-template-files/',
                        __('WordPress documentation', 'custom-post-types')
                    );
                }
                ...
                if (!isset($theme_compatibility_array[$current_theme_name]) && !empty($theme_single_template)) {
                    ...
                    if (!$theme_has_while) {
                        printf(
                            '<div class="%1$s"><p>%2$s<br><a href="%3$s" class="button-primary" style="margin-top:1em;" target="_blank">%4$s</a> <a href="%5$s" class="button-secondary" style="margin-top:1em;" target="_blank">%6$s</a></p></div>',
                            $this->cpt_ui_name . '-notices notice notice-error',
                            __('<strong>Custom post type</strong> notice:<br>The theme you are using does not contain the "while () : ... endwhile;" loop in the "single.php" file. To use custom templates the theme must have this "while" loop type in its "single.php" file.', 'custom-post-types'),
                            'https://wordpress.org/support/plugin/custom-post-types/',
                            __('Ask for integration', 'custom-post-types'),
                            'https://codex.wordpress.org/The_Loop#Loop_Examples',
                            __('WordPress documentation', 'custom-post-types')
                        );
                    }
                }
                ...
                if (!file_exists($current_teme_single_template) && ($theme_has_while || isset($theme_compatibility_array[$current_theme_name]))) {
                    printf(
                        '<div class="%1$s"><p>%2$s<br><a href="%3$s" class="button-primary" style="margin-top:1em;">%4$s</a></p></div>',
                        $this->cpt_ui_name . '-notices notice notice-error',
                        __('<strong>Custom post type</strong> notice:<br>Is this the first time you use the plugin or have you changed the theme?<br>In order to use custom templates, the plugin must copy the "single.php" file of the current theme to the "custom-templates" folder contained in the WordPress "uploads" folder.', 'custom-post-types'),
                        admin_url('edit.php?post_type=' . $this->cpt_ui_name . '_template&action=copy_theme_template'),
                        __('Continue', 'custom-post-types')
                    );
                }
            }
        }
